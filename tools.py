...
def calculate_uptime(start_time, end_time, device_name):
    """
    计算某个设备在指定时间段内的开机时长，以分钟为单位。
    Args:
        start_time: 查询的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time: 查询的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        shebeiname: 设备名称，只能是 '折臂吊车'、'A架'或'定位设备'。
    Returns:
        dict: 包括开机次数、总开机时长和开机时长列表的字典。若设备名称无效，则返回错误信息。
    """
    ...
    device_config = {
        ...
        "定位设备": ("database_in_use/Port3_ksbg_9.csv", "ON_DP", "OFF_DP"),
    }

    # 检查设备名称是否有效
    if device_name not in device_config:
        return {"error": f"计算失败，未知的设备名称: {device_name}"}
    ...
    for _, row in df_filtered.iterrows():
        if row["status"] == start_status:
            ...
            count += 1
            uptime_list.append(
                {f"第{count}次开机时长": f"{int(duration.total_seconds() / 60)}分钟"}
            )
    ...
    return {
        "开机次数": f"{count}次",
        "总开机时长": f"{int(total_duration.total_seconds() / 60)}分钟",
        "开机时长列表": uptime_list,
    }


@tool(parse_docstring=True)
def compute_operational_duration(start_time, end_time, device_name):
    """
    计算设备在指定时间段内的实际运行时长（有电流且不为0）。
    Args:
        start_time (str or pd.Timestamp): 开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str or pd.Timestamp): 结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        device_name (str, optional): 设备名称，只能为 "A架"。
    Returns:
        dict: 包含实际运行时长的字典。若设备名称无效，则返回错误信息。
    """
    ...
    return {
        "实际运行时长": f"{int(total_duration.total_seconds() / 60)}分钟",
    }
...
def get_table_data(table_name, start_time, end_time, columns=None, status=None):
    ...
    Args:
        table_name (str): 数据表名，不包含'.csv'后缀。
        start_time (str): 开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        columns (list): 需要查询的列名列表，如果为None，则返回所有列。默认为None。
        status (str): 需要筛选的状态（例如 "开机"、"关机"），如果为None，则不筛选状态。默认为None。
    Returns:
    ...
    try:
        ...
        return {"error": f"查询失败！数据表 {table_name} 不存在"}
    ...
    if status is not None:
        ...
        if filtered_data.empty:
            return {"error": f"在数据表 {table_name} 中未找到状态为 {status} 的数据"}
    ...
    if missing_columns:
        return {"error": f"列名 {missing_columns} 在数据表 {table_name} 中不存在"}
...
def calculate_total_deck_machinery_energy(start_time, end_time):
    """
    计算指定时间范围内甲板机械的能耗，包括折臂吊车、一号门架、二号门架、绞车，以及总能耗。A架代表一号门架和二号门架的组合。
    Args:
        start_time (str): 指定时间范围的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 指定时间范围的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        dict: 包含甲板机械四个部分（折臂吊车、一号门架、二号门架、绞车）的能耗和总能耗（kWh）的字典。若文件未找到、时间列转换失败或数据为空，则返回错误信息。
    """
    ...
    if (
        ...
        return {"error": "计算失败！所有的甲板机械设备数据为空。"}
...
@tool(parse_docstring=True)
def query_device_parameter(keyword):
    """
    通过参数中文名查询'设备参数详情表'中的设备参数信息。
    Args:
        keyword(str): 用来查询设备参数信息的关键字，多个关键字使用空格隔开，如'发电机组 温度'。
    Returns:
        dict: 包含设备参数信息的字典。若未找到相关参数，则返回错误信息。
    """
    # 读取设备参数详情表
    ...
    if not df["Channel_Text_CN"].str.contains(keyword).any():
        return {"result": (f"未找到包含 '{keyword}' 的参数，请减少关键字再查询。")}

    # 获取包含参数中文名的所有行
    parameter_infos = df[df["Channel_Text_CN"].str.contains(keyword)]
...
def calculate_total_energy_consumption(start_time, end_time):
    """
    计算指定时间范围内推进系统的能耗，包括一号推进变频器、二号推进变频器、艏推（也就是侧推或艏侧推）、可伸缩推和总能耗。
    Args:
        start_time (str): 指定时间范围的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 指定时间范围的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        dict: 包含推进系统四个部分（一号推进变频器、二号推进变频器、艏推、可伸缩推）的能耗和总能耗（kWh）的字典。若文件未找到、时间列转换失败或数据为空，则返回错误信息。
    """
    # 文件路径和功率列名
...
def get_device_status_by_time_range(start_time, end_time, device_name):
    """
    根据设备名称、开始时间和结束时间，查询某个设备在该时间段内的状态变化，并排除 status 为 'False' 的记录。
    Args:
        start_time (str): 开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    ...
    def get_status_changes(table_name, device_name):
        ...
        if "status" not in filtered_data.columns:
            return {"error": f"数据表 {table_name} 中不存在 'status' 列"}
    ...
    # 获取三个设备的状态变化
    if device_name == "A架":
        return get_status_changes("Ajia_plc_1", "A架")
    elif device_name == "折臂吊车":
        return get_status_changes("device_13_11_meter_1311", "折臂吊车")
    elif device_name == "定位设备":
        return get_status_changes("Port3_ksbg_9", "定位设备")
    else:
        return {"error": f"未知的设备名称: {device_name}"}


@tool(parse_docstring=True)
def calculate_generator_energy_consumption(start_time, end_time):
    """
    计算指定时间范围内发电系统的能耗，包括一号发电机组、二号发电机组、三号发电机组、四号发电机组和总能耗。
    Args:
        start_time (datetime): 要计算能耗的时间段的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (datetime): 要计算能耗的时间段的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        dict: 包含四个发电机组的能耗和总能耗（单位为kWh）的字典。若文件未找到、时间列转换失败或数据为空，则返回错误信息。
    """
...
def check_ajia_angle(start_time, end_time):
    """
    检查在指定时间范围内的 A 架角度异常数据，输出有异常数据的时间段。
    Args:
        start_time (datetime): 起始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (datetime): 结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        list: 包含异常时间段的元组列表，每个元组包含异常开始时间和结束时间。若文件未找到、时间列转换失败或数据为空，则返回错误信息。
    """
    ...
    try:
        ...
        return {"error": "查询失败！文件未找到。"}

    # 确保时间列是 datetime 类型
    try:
        df["csvTime"] = pd.to_datetime(df["csvTime"])
    except Exception:
        return {"error": "查询失败！时间列转换失败。"}
    ...
    for index, row in filtered_data.iterrows():
        if row["Ajia-0_v"] == "error" or row["Ajia-1_v"] == "error":
            ...
            error_time.append(
                {"异常开始时间": error_start_time, "异常结束时间": error_end_time}
            )
            error_status = False
            continue
    ...
    return {"异常时间段列表": error_time}


@tool(parse_docstring=True)
def calculate_fuel_consumption(start_time, end_time):
    """
    计算给定时间范围内发动系统的燃油消耗量，包括一号发电机组、二号发电机组、三号发电机组、四号发电机组和总消耗量。
    Args:
        start_time (datetime): 要计算燃油消耗的时间段的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (datetime): 要计算燃油消耗的时间段的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        dict: 包含四个发动机组的燃油消耗量和总燃油消耗量（L）的字典。若文件未找到、时间列转换失败或数据为空，则返回错误信息。
    """
...
def calculate_fuel_consumption_weight(volume, density):
    """
    根据燃油消耗量的体积与燃油密度，计算燃油消耗量的重量，以kg为单位。
    Args:
        volume (float): 燃油体积（L）。
...
def calculate_theoretical_energy_output(consumption, density, heating_value):
    ...
    return {"理论发电量": round(energy, 2)}


@tool(parse_docstring=True)
def get_field_dict():
    """
    获取在dict.json文件中，对所有数据表中字段进行说明的字段字典。
    Returns:
        dict: 包含数据表明、字段名和字段含义的字典。
    """
    with open("dict.json", "r", encoding="utf-8") as f:
        field_dict = json.load(f, encoding="utf-8")
    return field_dict
...
def get_work_time(start_time, end_time):
    """
    计算指定时间范围内，每次深海作业的持续时间，以字典的列表形式返回。
    Args:
        start_time (str): 要查询设备状态的时间范围的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 要查询设备状态的时间范围的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
    Returns:
        list of dict: 包含作业开始时间和结束时间的字典列表。
    """
    try:
        action_ajia = get_device_status_by_time_range.invoke(
            {"start_time": start_time, "end_time": end_time, "device_name": "A架"}
        )["正在进行的关键动作"]
        action_dp = get_device_status_by_time_range.invoke(
            {"start_time": start_time, "end_time": end_time, "device_name": "定位设备"}
        )["正在进行的关键动作"]
    except Exception:
        return {"error": "没有进行作业。"}
    ...
    # 添加最后一个合并后的时间片段
    merged_time_sections.append({"作业开始": current_start, "作业结束": current_end})

    return merged_time_sections
...
def find_min_value(start_time, end_time, table_name, column_name):
    ...
    Args:
        start_time (str): 要查询的时间范围的开始时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        end_time (str): 要查询的时间范围的结束时间，格式为 'YYYY-MM-DD HH:MM:SS'。
        table_name (str): 数据表名，要带上'.csv'后缀。
        column_name (str): 列名。
    Returns:
        dict: 包含最小值和对应时间的字典。
    """
    ...
    return {"最小值": min_value, "对应时间": min_time}
...
def find_max_value(start_time, end_time, table_name, column_name):
    ...
    return {"最大值": max_value, "对应时间": max_time}
...
def calculate_total_rudder_energy(start_time, end_time):
    ...
    Returns:
        dict: 包括一号船舵A、一号船舵B、二号船舵A、二号船舵B和总体的能耗（kWh）的字典。若文件未找到，则返回错误信息。
    """
    # 文件路径和功率列名
    files_and_columns = {
