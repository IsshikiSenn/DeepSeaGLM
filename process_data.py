            ...
            if steady:  # 之前状态为稳定值
                if not is_steady_value(value):  # 此时刻为非稳定值，改为上升状态
                    if index == end_index - 1:  # 事件结束，标记峰值
                        df_ajia.loc[index, "current_status"] = "上升值 峰值"
                    else:
                        steady = False
                ...
                if not is_steady_value(value):  # 此时刻为非稳定值，继续上升状态
                    if value > value_max:  # 更新峰值
                        ...
                        index_max = index
                    if index == end_index - 1:  # 事件结束，标记峰值
                        df_ajia.loc[index_max, "current_status"] = "峰值" if df_ajia.loc[index_max, "current_status"] == "False" else "上升值 峰值"
                else:  # 此时刻为稳定值，改为稳定状态
                    df_ajia.loc[index_max, "current_status"] = "峰值" if df_ajia.loc[index_max, "current_status"] == "False" else "上升值 峰值"  # 标记峰值
                    steady = True
